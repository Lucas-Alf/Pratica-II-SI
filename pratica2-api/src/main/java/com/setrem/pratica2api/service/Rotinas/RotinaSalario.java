...
public class RotinaSalario {
    public List<IncidenciaDTO> Calcula(EventoCalculoDTO evento, List<IncidenciaDTO> incidencias, int contrato) {
        for (int incidenciaAtingida : evento.getIncidenciasAtingidas()) {
            if (incidencias.stream().filter(x -> x.getId() == incidenciaAtingida).collect(Collectors.toList())
                ...
                if (evento.getTipo() == "V") {
                    // incidenciaEncontrada.setValor(incidenciaEncontrada.getValor() +
                    // evento.getValor());
                } else {
                    // incidenciaEncontrada.setValor(incidenciaEncontrada.getValor() -
                    // evento.getValor());
