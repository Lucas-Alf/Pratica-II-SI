...
export class EnderecoComponent implements OnInit {
  ...
  alterar(): void {
    if (this.selection.selected.length > 0) {
      ...
      this.dialogRef = this.dialog.open(EnderecoModalComponent, { data: { action: 'Alterar', component: this, info: selection } });
    } else {
  ...
  excluir(): void {
    if (this.selection.selected.length > 0) {
      this.loaderService.show();
      axios.delete(this.apiUrl + 'endereco/delete/' + this.selection.selected[0].id).then((response) => {
